    ...
    <main className="min-h-screen bg-[#f7f9f8] text-[#101512]">
      ...
      <section className="mx-auto grid max-w-[1200px] gap-6 px-5 py-8 sm:px-8 lg:grid-cols-[230px_1fr] lg:px-10">
        <aside className="h-fit rounded-2xl border border-[#dfe7e2] bg-white p-2 shadow-sm">
          {[['profile','Profile'],['security','Security'],['notifications','Notifications'],['preferences','Preferences']].map(([id,label])=><button key={id} onClick={()=>setTab(id)} className={`mb-1 w-full rounded-xl px-4 py-3 text-left text-[13px] font-bold ${tab===id?'bg-[#edf8f1] text-[#087f3f]':'text-[#4f5955] hover:bg-[#f7f9f8]'}`}>{label}</button>)}
        </aside>
        <div className="space-y-5">
          ...
          {tab==='notifications' && <div className="rounded-2xl border border-[#dfe7e2] bg-white p-6 shadow-sm"><h2 className="text-xl font-black">Notifications</h2>{[['Account notifications',notifications,setNotifications],['Product & market updates',marketing,setMarketing]].map(([label,value,setter])=><div key={String(label)} className="flex items-center justify-between border-b border-[#edf0ee] py-5"><div><p className="text-[13px] font-bold">{String(label)}</p><p className="mt-1 text-[12px] text-[#68726e]">Receive relevant updates by email.</p></div><button onClick={()=>setter(!(value as boolean))} className={`h-6 w-11 rounded-full p-1 ${value?'bg-[#087f3f]':'bg-[#cbd3ce]'}`}><span className={`block h-4 w-4 rounded-full bg-white transition ${value?'translate-x-5':''}`} /></button></div>)}</div>}
