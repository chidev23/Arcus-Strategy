...
export default function AdminSidebar() {
  ...
  const [open, setOpen] = useState<Record<string, boolean>>({ Customers: true, Services: true, Trading: true, Subscriptions: true, Finance: true, Content: true, Support: true, Tickets: true });
  ...
  return <aside className="hidden min-h-screen w-[280px] shrink-0 border-r border-[#dfe6e2] bg-white lg:flex lg:flex-col">
    <div className="border-b border-[#e7ece9] px-5 py-5"><Link href={withRole(support ? '/admin/support' : '/admin')} className="flex items-center gap-3"><span className="grid h-10 w-10 place-items-center rounded-xl bg-[#087f3f] text-sm font-black text-white">A</span><span><strong className="block text-[14px] tracking-[.16em] text-[#101512]">{support ? 'ARCUS SUPPORT' : 'ARCUS STRATEGY'}</strong><small className="block text-[8px] font-black tracking-[.28em] text-[#087f3f]">{support ? 'SUPPORT OPERATIONS' : 'ADMINISTRATION'}</small></span></Link></div>
