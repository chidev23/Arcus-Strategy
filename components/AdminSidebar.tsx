...
import { Activity, ArrowLeft, Bell, BookOpen, BriefcaseBusiness, ChartNoAxesCombined, Check, ChevronDown, ChevronRight, FileBarChart, FilePenLine, Headphones, LayoutDashboard, LineChart, Megaphone, MessageSquareWarning, ScrollText, Settings2, ShieldAlert, UserCog, Users, WalletCards, CreditCard } from 'lucide-react';
import { ADMIN_NAVIGATION, ADMIN_ROLES, AdminRole } from '../lib/admin-rbac';

const icons: Record<string, any> = { 'layout-dashboard': LayoutDashboard, users: Users, 'briefcase-business': BriefcaseBusiness, 'chart-no-axes-combined': ChartNoAxesCombined, 'credit-card': CreditCard, 'wallet-cards': WalletCards, 'file-pen-line': FilePenLine, headphones: Headphones, 'user-cog': UserCog, 'file-bar-chart': FileBarChart, 'scroll-text': ScrollText, 'settings-2': Settings2, 'message-square-warning': MessageSquareWarning, 'book-open': BookOpen, megaphone: Megaphone, 'shield-alert': ShieldAlert, activity: Activity, 'line-chart': LineChart, bell: Bell };
...
export default function AdminSidebar() {
  ...
  const [open, setOpen] = useState<Record<string, boolean>>({ Customers: true, Services: true, Trading: true, Subscriptions: true, Finance: true, Content: true, Support: true, Tickets: true, Accounts: true, Performance: true, Reports: true });
  ...
  const analyst = role === 'financial_analyst';

  return <aside className="hidden min-h-screen w-[280px] shrink-0 border-r border-[#dfe6e2] bg-white lg:flex lg:flex-col">
    <div className="border-b border-[#e7ece9] px-5 py-5"><Link href={withRole(analyst ? '/admin/analytics' : support ? '/admin/support' : '/admin')} className="flex items-center gap-3"><span className="grid h-10 w-10 place-items-center rounded-xl bg-[#087f3f] text-sm font-black text-white">A</span><span><strong className="block text-[14px] tracking-[.16em] text-[#101512]">{support ? 'ARCUS SUPPORT' : analyst ? 'ARCUS ANALYTICS' : 'ARCUS STRATEGY'}</strong><small className="block text-[8px] font-black tracking-[.28em] text-[#087f3f]">{support ? 'SUPPORT OPERATIONS' : analyst ? 'ANALYTICS OPERATIONS' : 'ADMINISTRATION'}</small></span></Link></div>
