...
import { BadgeCheck, CircleUserRound, Headphones, Home, Landmark, Menu, Network, ShieldCheck, Star, X, KeyRound } from 'lucide-react';
...
export default function CustomerMenu() {
  ...
  const isBrokerPage = serviceParts[0] === 'services' && serviceParts[1] === 'social-trading' && serviceParts.length >= 4;
  const brokerSlug = isBrokerPage ? serviceParts[3] : '';
  return <div className="relative flex items-center gap-2">
    {isBrokerPage && <Link href={`/investor-password/${brokerSlug}?platform=${encodeURIComponent(serviceParts[2]?.toUpperCase() || 'MT5')}&type=Copy%20Trading`} className="hidden items-center gap-2 rounded-xl bg-[#087f3f] px-4 py-2.5 text-[10px] font-black text-white shadow-sm transition hover:bg-[#066c35] sm:inline-flex"><KeyRound size={14}/> View investor password</Link>}
